...
// (b) Método Push
  template <class T> void push(const T& dato) {
...
// (c) Método Pop
  template <class T> void pop(void) {
    assert(!empty());
    delete l_.extract_head();
  }

// (d) Método Top
  template <class T> T* top(void) {
    assert(!empty());
    return l_.get_head()->get_data();
  }

// (e) Método Empty
  template <class T> bool empty(void) {
    return l_.empty();
  }
};
...
//////////////////////////////////////////////
template <class T>
block_t<T> op1(const block_t<T> A, const block_t<T> B, const block_t<T> C) {
