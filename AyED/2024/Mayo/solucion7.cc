...
// valorará la eficiencia del procedimiento.
  ...
  #define LENGTH 4
  void write(const vector<int>& number) {
    for (int i : number) {
      cout << number[number.size() - 1 - i] << " ";
    }
    cout << endl;
  }
...
template <class T>
void write_direct(ostream& os) {
  write2(head_, os);
}

void write2(sll_node_t<T>* node, ostream& os) {
  if (node != NULL) {
    os << node->get_data();
...
template <class T>
void write_reverse_iterative(ostream& os) {
