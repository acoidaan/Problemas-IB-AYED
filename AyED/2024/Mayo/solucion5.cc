...
void write(const vector<int>& v, ostream& os)  { // const
...
// en orden inverso.
template <class T>
void sll_t<T>::write_reverse(ostream& os) { // const
  write1(head_, os);
}
...
template <class T>
void sll_t<T>::write_direct(ostream& os) const {
  write2(head_, os);
}
...
int vector<int>::BBI(const int x) const {
