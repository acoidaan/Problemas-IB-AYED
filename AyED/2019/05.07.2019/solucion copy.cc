...
double matrix_t<double>::TSD(void) {
  double sum = 0.0;
...
// a12, a13, a23
double matrix_t<double>::TSND(void) {
  double sum = 0.0;
...
double matrix_t<double>::TID(void) {
  int sum = 0.0;
  for (int i = 1; i <= get_m(); i++) {
    for (int j = 1; j <= i; j++) {
      sum += at(i, j);
...
// a21, a31, a32
double matrix_t<double>::TISD(void) {
  int sum = 0.0;
...
int bsearch(const int *v, int i, int d, int x) {
  if (i <= d) {
    int c = (i + d) / 2;
    if (v[c] == x) return c;
    if (v[c] < x) c = bsearch(v, c + 1, d, x); 
    if (v[c] > x) c = bsearch(v, i, c - 1, x); 
  }
  return -1;
}
...
int rsearch(const int *v, int i, int d, int x) {
  ...
  if (i <= d) {
    c = (i + d) / 2;
    if (v[c] != x) {
      const int c1 = rsearch(v, c + 1, d, x);
      const int c2 = rsearch(v, i, c - 1, x);
      c = max(c1, c2);
    }
  }
  return c;
...
template <class T>
void sll_t<T>::fusion_asc(sll_t<T>& a, sll_t<T>& b) {
  sll_node_t<T>* ptr_a = a.get_head();
  sll_node_t<T>* ptr_b = b.get_head();
  stack_l_t<T> stack;
...
// (b) (1.2) ¿Qué código habría que añadir al desarrollado en (a) para poder hacer una fusión en orden descendente? ¿Usarías algún TAD auxiliar? En tal caso, ¿cuál sería? Añadir el código necesario para hacer dicha fusión en orden descendente para crear el siguiente método: RESPUESTA: Quitando la pila ya funcionaría
...
double sparse_vector_t<double>::scal_prod(const vector_t<double>& b, const int i = 0) {
  assert(sz_ == b.get_sz());
  if (i == v_.get_sz()) return 0;
  return v_.get_v(i).get_val() * b.get_val(v_.get_v(i).get_inx) + scal_prod(b, i++);
}